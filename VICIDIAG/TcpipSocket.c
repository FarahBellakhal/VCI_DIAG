...
void Tester_deconnect(void)
{
    int result = close(sockfd);
     if( result < 0 )
        {
               printf("Error: deconnect Failed \n");
        printf("Reason: %s\n", strerror(errno));
        exit(0);
        }else{
        printf("deconnected from %s, and PORT: %d \n", inet_ntoa(serv_addr.sin_addr), ntohs(serv_addr.sin_port));
    }
    ...
    if((n=read(sockfd, Response, sizeof(Response) - 1)) > 0){
        ...
        length = strlen(Response);
           for(i = length--; i >= 0; i--)
           {
               ...
               {
                   decimal += (Response[i] - 48) * base;
                   base *= 16;
               }
               else if(Response[i] >= 'A' && Response[i] <= 'F')
               {
                   decimal += (Response[i] - 55) * base;
                   base *= 16;
               }
               else if(Response[i] >= 'a' && Response[i] <= 'f')
               {
                   decimal += (Response[i] - 87) * base;
                   base *= 16;
               }
           }
        Response[n] = 0;
...
void tcpipSocket_close(void) {
  close(sockfd);
    printf("Deconnected from %s", inet_ntoa(serv_addr.sin_addr));
}
